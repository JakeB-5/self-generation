...
**Current status**: Design phase only — `DESIGN.md` (2880 lines, v7) is the complete implementation spec. No source code has been implemented yet. All code in DESIGN.md is the reference implementation to follow.

## Tech Stack

- **Runtime**: Node.js >= 18, ES Modules (`.mjs`)
- **Storage**: JSONL (append-only), no external DB dependencies
- **Analysis**: `claude --print` for semantic pattern analysis
- **Hook system**: Claude Code Hooks API (registered in `~/.claude/settings.json`)
- **Dependencies**: Zero external npm packages — pure Node.js built-in modules only (`fs`, `path`, `child_process`)
...
├── data/
│   ├── prompt-log.jsonl           # Global event log (all projects, all sessions)
│   ├── feedback.jsonl             # Suggestion accept/reject records
│   ├── analysis-cache.json        # AI analysis result cache
│   └── error-kb.jsonl             # Error resolution history KB
...
│   ├── session-summary.mjs        # SessionEnd: summary + AI analysis trigger
│   └── session-analyzer.mjs       # SessionStart: cache injection + context
├── lib/                           # 5 utility modules
│   ├── log-writer.mjs             # JSONL read/write utilities
│   ├── ai-analyzer.mjs            # claude --print wrapper + cache management
│   ├── error-kb.mjs               # Error KB search/record
│   ├── skill-matcher.mjs          # Prompt-to-skill matching
...
- **Global-first, project-filtered**: All events go to a single `prompt-log.jsonl` with `project` and `projectPath` fields for filtering
- **Non-blocking hooks**: All hooks exit 0 quickly (timeout: 2s for collection, 5s for analysis)
- **Schema versioned**: All JSONL entries include `v: 1` field
- **Privacy-conscious**: Only first word of Bash commands stored; full args not logged

### Implementation Phases

1. **Phase 1 — Data Collection**: Hook scripts that log events to `prompt-log.jsonl`
