...
export async function searchErrorKB(normalizedError) {
  ...
  try {
    ...
    // Medium confidence: 0.76-0.85 (keyword verification required per REQ-RA-002)
    ...
    const commonKeywords = queryWords.filter(w => matchWords.includes(w));

    if (commonKeywords.length === 0) return null; // No common keywords, reject match
...
export function recordResolution(normalizedError, details) {
  ...
  try {
    const db = getDb();
    // Accept both camelCase (from hooks) and snake_case (from direct calls)
    const errorRaw = details.errorRaw || details.error_raw;
    const resolvedBy = details.resolvedBy || details.resolved_by;
    const toolSequence = details.toolSequence || details.tool_sequence;
    // Stringify array for SQL TEXT column
    const toolSeqStr = Array.isArray(toolSequence) ? JSON.stringify(toolSequence) : (toolSequence || null);
